...
proportions <- plastics %>%
  group_by(year) %>%
  summarise(across(plastic_types, sum, na.rm = TRUE),
            total_all = sum(grand_total, na.rm = TRUE))

map(plastic_types, \(plastic) {
  tibble(
    plastic_type = plastic,
    prop_2019 = proportions[[plastic]][proportions$year == 2019] / proportions$total_all[proportions$year == 2019],
    prop_2020 = proportions[[plastic]][proportions$year == 2020] / proportions$total_all[proportions$year == 2020]
  ) %>%
    mutate(direction = if_else(prop_2020 > prop_2019, "increased", "decreased"))
}) %>%
  bind_rows()
```
